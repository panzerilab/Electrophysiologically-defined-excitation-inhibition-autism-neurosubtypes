function tab2output = compute_features(datafile, outdir, f_range, f_range_fooof)
...
for iwin = 1:baseline_windows
    [coeff, score, latent, tsquared, explained] = pca(squeeze(data.baseline(:,:,iwin)));
%     disp(explained(1));
...
for iwin = 1:length(transition_windows)
    ...
    [coeff, score, latent, tsquared, explained] = pca(squeeze(data.cno(:,:,window2use)));
%     disp(explained(1));
