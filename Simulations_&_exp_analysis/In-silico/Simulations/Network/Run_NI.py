"""
2022-11-23
In this script we run simulation of E-I network with PV silencing manipulation
"""
import os, sys
import numpy as np
import pandas as pd
...
root_results = os.path.join("..", "Simulation results", "NI")
...
modulation_factor = np.linspace(0.3, 3, 5)

for m, el in product(modulation_factor):

    d_el = el - (-70)
    new_params = tools.dictupdate(base_params, {

        "N_inh": base_params["inh_inh_recurrent"] * m,
        "inh_inh_recurrent": base_params["inh_inh_recurrent"] / m,
        "inh_exc_recurrent": base_params["inh_exc_recurrent"] / m,
